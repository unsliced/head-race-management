		...
		public static void Dump(IEnumerable<ICrew> crews) 
		{
			...
			using(var fs = new FileStream("Vets Head 2014 Results.pdf", FileMode.Create)){
				using(Document document = new Document(PageSize.A4.Rotate())){
					...
					float[] widths = new float[] { 1f, 1f, 8f, 1f, 3f, 1f, 1f, 1f };
					...
					foreach(var h in new List<string> { "Overall", "Start", "Crew", "Elapsed", "Category", "Category Pos", "Gender Pos", "Foreign Pos" })
					{
					...
					foreach (var crew in crews.OrderBy(cr => cr.FinishType).ThenBy(cr => cr.Elapsed)) 
					{
						...
						if (crew is UnidentifiedCrew)
							...
							else 
							{
								...
								{
									overall = crew.CategoryPosition (crew.Categories.First (c => c is OverallCategory)).ToString ();
									categorypos = crew.CategoryPosition (crew.Categories.First (c => c == primary)).ToString ();
									genderpos = crew.CategoryPosition (crew.Categories.First (c => c.EventType == EventType.MastersHandicapped)).ToString ();
									categorypos = crew.CategoryPosition (crew.Categories.First (c => c == primary)).ToString ();
									var foreign = crew.Categories.FirstOrDefault (c => c.EventType == EventType.Foreign);
									if (foreign != null)
										foreignpos = crew.CategoryPosition (foreign).ToString ();
								}
							}

						var objects = new List<Tuple<string, Font>> { 
							new Tuple<string, Font> (overall, font),
							new Tuple<string, Font> (crew.StartNumber.ToString (), font),
							new Tuple<string, Font> (crew.Name, font),
							new Tuple<string, Font> (crew.FinishType == FinishType.Finished ? crew.Elapsed.ToString().Substring(3).Substring(0,8) : crew.FinishType.ToString(), font),
